 ...
 */
public class OBS_SME_BasicKMeansInterface implements SME_KMeans_Paralel {
     ...
     */
    public OBS_SME_BasicKMeansInterface(double[][] coordinates, int k, int maxIterations,
                                        long randomSeed) {
     ...
     */
    private void postKMeansComplete(SME_Cluster[] clusters, long executionTime) {
        if (mListeners.size() > 0) {
            synchronized (mListeners) {
                int sz = mListeners.size();
                for (int i=0; i<sz; i++) {
    ...
    private SME_Cluster[] generateFinalClusters() {
        ...
        List<SME_Cluster> clusterList = new ArrayList<SME_Cluster>(numClusters);
        for (int c = 0; c < numClusters; c++) {
            ProtoCluster pcluster = mProtoClusters[c];
            if (!pcluster.isEmpty()) {
                SME_Cluster cluster = new SME_Cluster(pcluster.getMembership(), pcluster.getCenter());
