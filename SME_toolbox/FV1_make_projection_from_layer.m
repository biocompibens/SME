...
Img3=uint8(255*mat2gray(Img3));

Img3=cat(3,uint8(zeros(size(Img3,1),size(Img3,2),up)),uint8(Img3),uint8(zeros(size(Img3,1),size(Img3,2),low)));

%    zprojf1=uint8(uint8(zeros(size(Img1,1),size(Img1,2))));
zprojf11=[];
for int=1:up+low+1
   zprojf11(:,:,int)=uint8(zeros(size(Img3,1),size(Img3,2)));
end
